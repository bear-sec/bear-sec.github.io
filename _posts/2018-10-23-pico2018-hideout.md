...
in these sorts of challenges the first go-to bug to check is wether of not i can bet more than i have, by supplying a huge number that will be compared and interpreted as a signed negative number, and pass the check at line 56 in get_bet function. and indeed its vulnerable. 
...
just running rand() 3 times wont work too, cause if we see colsely in the code, between roulette spins, if the choice correct, it calls rand() again to get a random congratulations message. so we need to get the 1st, 3rd and 5th values.
Now that we have our streak, we can get the 7th value of rand and supply our bet as MAX_INT+ONE_BILLION and we get the flag.
