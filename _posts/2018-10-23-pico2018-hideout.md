...
We can see that the file opened is "./permission.txt" which reffers to a file in current directory, which we dont have control over, but if we create a file with the same name in our home dir, we can run the program and get the pass the check. 
